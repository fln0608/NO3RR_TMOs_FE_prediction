...
os.makedirs('My_saved_pipelines_NN2', exist_ok=True)
...
class Model(nn.Module):
    def __init__(self, input_size):
        ...
        self.net = nn.Sequential(
            nn.Linear(input_size, 202), nn.ReLU(), nn.Dropout(0.1),
            nn.Linear(202, 205), nn.ReLU(),  nn.Dropout(0.1),
            nn.Linear(205, 1), nn.Sigmoid()  
        )
...
for fold, (train_idx, val_idx) in enumerate(kf.split(x_data)):
    ...
    joblib.dump(preprocessor, f'My_saved_pipelines_NN2/preprocessor_fold{fold}.pkl')
    ...
    # Save the model
    torch.save(model.state_dict(), f'My_saved_pipelines_NN2/model_fold{fold}.pth')
